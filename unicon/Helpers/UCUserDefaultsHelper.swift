...
class UCUserDefaultsHelper {
    ...
    public static func getMyTeams() {
        let key = "myTeams"
        MatchingViewController.myTeamList = ud.array(forKey: key)  as? [String] ?? []
    }
    
    public static func setMyTeams() {
        let key = "myTeams"
        ud.set(MatchingViewController.myTeamList, forKey: key)
    }
}
